...
def create_new_task(task_data:TaskSchema,db:Session=Depends(get_db),user:UserModel=Depends(is_authenticated)):
    return controller.create_task(task_data,db,user)
...
def get_one_tasks(task_id:int , db:Session=Depends(get_db),user:UserModel=Depends(is_authenticated)):
...
def update_task(task_data:TaskSchemaWithId,db:Session=Depends(get_db),user:UserModel=Depends(is_authenticated)):
